...
/// <summary>
/// TASK-013 — قائمة مرتجعات المشتريات مع فلاتر البحث
/// </summary>
internal sealed class PurchaseReturnsView : UserControl
{
    ...
    private List<PurchaseReturn>    _all = new();
    ...
    public PurchaseReturnsView()
    {
        ...
        _statusCombo.Items.AddRange(new object[] { "الكل", "مسودة", "معتمد" });
    ...
    private Control BuildToolbar()
    {
        ...
        _newBtn.BackColor = AppTheme.Danger;
        _newBtn.Width = 160; _newBtn.Margin = new Padding(4, 4, 4, 0);

        AppTheme.StyleSecondaryButton(_openBtn);
        _openBtn.Width = 110; _openBtn.Margin = new Padding(4, 4, 4, 0);

        AppTheme.StyleSecondaryButton(_searchBtn);
        _searchBtn.Width = 90; _searchBtn.Margin = new Padding(4, 4, 4, 0);
    ...
    private Control BuildGrid()
    {
        ...
        _grid.ColumnHeadersDefaultCellStyle.BackColor = AppTheme.Danger;
        ...
        _grid.Columns.Add(new DataGridViewTextBoxColumn { Name = "colNum",    HeaderText = "رقم المرتجع",   FillWeight = 12 });
        _grid.Columns.Add(new DataGridViewTextBoxColumn { Name = "colDate",   HeaderText = "التاريخ",       FillWeight = 10 });
        _grid.Columns.Add(new DataGridViewTextBoxColumn { Name = "colInv",    HeaderText = "الفاتورة الأصلية", FillWeight = 12 });
        ...
        _grid.Columns.Add(new DataGridViewTextBoxColumn { Name = "colTotal",  HeaderText = "إجمالي المرتجع", FillWeight = 12 });
        _grid.Columns.Add(new DataGridViewTextBoxColumn { Name = "colStatus", HeaderText = "الحالة",         FillWeight = 8 });
        _grid.Columns.Add(new DataGridViewTextBoxColumn { Name = "colNotes",  HeaderText = "ملاحظات",        FillWeight = 18 });
    ...
    private Control BuildFooter()
    {
        ...
        _feedbackLbl.Text = "انقر نقراً مزدوجاً لفتح المرتجع — المرتجعات المعتمدة للعرض فقط.";
    ...
    {
        _newBtn.Click         += (_, _) => OpenReturn(null);
    ...
    private void PopulateGrid()
    {
        ...
        foreach (var r in _all)
        {
            int idx = _grid.Rows.Add(
                ...
                string.IsNullOrEmpty(r.OriginalInvoiceNum) || r.OriginalInvoiceNum == "—"
                    ? "مرتجع مستقل" : r.OriginalInvoiceNum,
                r.SupplierName,
                r.WarehouseName,
    ...
    private void OpenSelected()
    {
        ...
        var ret = _all.FirstOrDefault(r => r.Id == id);
        if (ret is null) return;
        try
        {
            var full = new PurchaseReturnRepository().GetById(ret.Id);
            OpenReturn(full);
        }
        catch (Exception ex) { ShowError(ex.Message); }
    }

    private void OpenReturn(PurchaseReturn? ret)
    {
        using var dlg = new PurchaseReturnDialog(ret);
        if (dlg.ShowDialog() == DialogResult.OK)
        {
            LoadData();
            ShowSuccess("تم تحديث قائمة المرتجعات.");
        }
    }
