...
/// <summary>كشف حساب المورد — فواتير وأرصدة تفصيلية</summary>
internal sealed class SupplierStatementDialog : Form
{
    ...
    private void BuildUI()
    {
        ...
        root.RowStyles.Add(new RowStyle(SizeType.Absolute, 54F));   // header info
        root.RowStyles.Add(new RowStyle(SizeType.Absolute, 50F));   // date filter
        root.RowStyles.Add(new RowStyle(SizeType.Percent, 100F));   // grid
        root.RowStyles.Add(new RowStyle(SizeType.Absolute, 60F));   // totals
    ...
    private Control BuildInfoBar()
    {
        ...
        var parts = new List<string> { _supplier.Name };
        if (!string.IsNullOrEmpty(_supplier.Phone))  parts.Add($"📞 {_supplier.Phone}");
        if (!string.IsNullOrEmpty(_supplier.Mobile)) parts.Add($"📱 {_supplier.Mobile}");
        if (_supplier.CreditLimit > 0)               parts.Add($"حد الائتمان: {_supplier.CreditLimit:N2}");
    ...
    private Control BuildTotalsBar()
    {
        ...
        for (int i = 0; i < 6; i++)
            layout.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 16.6F));
        ...
        void AddTotal(Label lbl, string caption, Color color, int col)
        {
            ...
            cell.RowStyles.Add(new RowStyle(SizeType.Percent, 45F));
            cell.RowStyles.Add(new RowStyle(SizeType.Percent, 55F));
        ...
        layout.Controls.Add(_feedbackLabel, 5, 0);
    ...
    private void LoadStatement()
    {
        try
        {
            var stmt = new SupplierRepository()
                .GetStatement(_supplier.Id, _fromPicker.Value.Date, _toPicker.Value.Date);
