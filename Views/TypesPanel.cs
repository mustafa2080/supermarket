...
//  لوحة أنواع الأصناف
// ══════════════════════════════════════════════════════════════
internal sealed class TypesPanel : UserControl
{
    ...
    private List<ItemType>        _all = new();

    public TypesPanel()
    {
        _grid          = new DataGridView();
        _searchBox     = AppTheme.CreateTextBox("بحث باسم النوع...");
        _addBtn        = new Button { Text = "إضافة نوع" };
    ...
    private Control BuildGrid()
    {
        ...
        _grid.Columns.Add(new DataGridViewTextBoxColumn { Name = "colNameAr",HeaderText = "اسم النوع",   FillWeight = 40 });
    ...
    private void LoadData()
    {
        try
        {
            _all = new CategoryRepository().GetAllTypes();
            PopulateGrid(_all);
            ShowInfo($"تم تحميل {_all.Count} نوع.");
        }
        catch (Exception ex) { ShowError(ex.Message); }
    }

    private void PopulateGrid(List<ItemType> items)
    {
        _grid.Rows.Clear();
        foreach (var t in items)
        {
            int idx = _grid.Rows.Add(
                ...
                t.ItemCount > 0 ? t.ItemCount.ToString() : "—",
    ...
    private void ApplyFilter()
    {
        ...
        var filtered = string.IsNullOrEmpty(q)
            ? _all
            : _all.Where(t =>
                t.NameAr.Contains(q, StringComparison.OrdinalIgnoreCase) ||
                t.Name.Contains(q, StringComparison.OrdinalIgnoreCase)).ToList();
        PopulateGrid(filtered);
    }

    private ItemType? GetSelected()
    {
    ...
    {
        var t = GetSelected();
        if (t is null) { ShowError("اختر نوعاً أولاً."); return; }
        OpenDialog(t);
    }
    ...
    private void DeleteSelected()
    {
        ...
        if (t.ItemCount > 0)
        {
            ShowError($"لا يمكن الحذف — النوع مرتبط بـ {t.ItemCount} صنف.");
            return;
        }

        var confirm = MessageBox.Show(
            $"هل تريد حذف النوع [{t.NameAr}] نهائياً؟",
            "تأكيد الحذف", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
        if (confirm != DialogResult.Yes) return;

        try
        {
            var (ok, err) = new CategoryRepository().DeleteType(t.Id);
            if (!ok) { ShowError(err); return; }
            AuditService.LogDelete("public.item_types", t.Id);
            LoadData();
            ShowSuccess("تم حذف النوع بنجاح.");
        }
        catch (Exception ex) { ShowError(ex.Message); }
    }

    private void OpenDialog(ItemType? type)
    {
        using var dlg = new TypeDialog(type);
        if (dlg.ShowDialog() == DialogResult.OK)
        {
            LoadData();
            ShowSuccess(type is null ? "تمت إضافة النوع بنجاح." : "تم تحديث النوع.");
        }
    }
